...
export async function doRefresh(): Promise<string> {
  ...
  inflight = (async () => {
    ...
    try {
      // Body truyền lên bắt buộc dạng snake_case: { "refresh_token": localStorage.getItem('rt_token') }
      ...
      // Response trả về có dạng: res.data.data.accessToken và res.data.data.refreshToken
      ...
      // Lưu refreshToken mới vào LocalStorage với key 'rt_token'
      if (refreshToken) {
