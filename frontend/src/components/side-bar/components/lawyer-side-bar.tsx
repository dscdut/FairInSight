...
import { lawyerSideBarLinks } from '@/core/constants/general.const'
...
export default function LawyerSideBar() {
  ...
  const isActiveLink = useCallback((linkPath: string) => {
    ...
    if (linkPath === `${ROUTE.LAWYER.ROOT}/${ROUTE.LAWYER.DASHBOARD}`) {
      return currentPath === `${ROUTE.LAWYER.ROOT}/${ROUTE.LAWYER.DASHBOARD}` || currentPath === ROUTE.LAWYER.ROOT
    }
  ...
  return (
    <aside
      ...
      aria-label='Lawyer Sidebar navigation'
    >
      {/* Header */}
      <div
        className={cn(
      ...
      <nav
        ...
        role='navigation'
      >
        {lawyerSideBarLinks.map((link, index) => {
